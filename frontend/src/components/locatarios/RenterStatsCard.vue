<template>
  <q-card flat bordered class="stat-card" :class="`stat-card-${cardIndex}`">
...
defineProps({
  ...
  type: {
    ...
    required: true,
    validator: (value) =>
      ["total", "with-rentals", "without-rentals"].includes(value),
  },
});
</script>
...
.stat-icon-with-rentals {
  background: #e5efe6;
  color: #2e7a3a;
}

.stat-icon-without-rentals {
  background: #ece9e1;
  color: #5d5546;
}
