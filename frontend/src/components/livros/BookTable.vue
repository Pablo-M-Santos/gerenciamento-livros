    ...
    >
      ...
      <template #body-cell-actions="props">
        <q-td :props="props" class="actions-cell">
          ...
          >
            <q-tooltip>Editar livro</q-tooltip>
          </q-btn>

          <q-btn
            v-if="isAdmin && !isBookWithRentals(props.row.id)"
            flat
            round
            dense
            icon="delete"
          ...
          >
            <q-tooltip>Excluir livro</q-tooltip>
          </q-btn>
        </q-td>
      </template>

      <template #no-data>
        <div class="q-pa-lg text-grey-7">Nenhum livro encontrado.</div>
      </template>
    </q-table>
  </q-card>
</template>
...
const isBookWithRentals = (bookId) => props.booksWithRentals.includes(bookId);
</script>
